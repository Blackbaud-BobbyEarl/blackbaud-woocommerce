...
/**
* Blackbaud WooCommerce BBSP Payment Gateway
*
* Adds BBSP as a payment gateway for WooCommerce.
*
* @class Blackbaud_WooCommerce_BBSP_Gateway
* @extends WC_Payment_Gateway
* @author Blackbaud - Bobby Earl
* @version 1.0.0
*/
class Blackbaud_WooCommerce_BBSP_Gateway extends WC_Payment_Gateway {
	...
	*/
	public function __construct() {
		...
		$this->method_title = __('BBSP', 'woocommerce');
		$this->method_description = __('BBSP Payment Gateway', 'woocommerce');
	...
	function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				...
				'type' => 'checkbox',
				'label' => __('Enable BBSP Payment Gateway', 'woocommerce'),
				'default' => 'yes'
			),
			'agent_code' => array(
				'title' => __('BBSP Agent Code', 'woocommerce'),
				'type' => 'text',
				'description' => __('Required agent code provided by BBSP.', 'woocommerce'),
				'desc_tip' => true
			),
			'password' => array(
				'title' => __('BBSP Password', 'woocommerce'),
				'type' => 'password',
				'description' => __('Required password provided by BBSP.', 'woocommerce'),
				'desc_tip' => true
			),
			'title' => array(
			...
			'description' => array(
				...
				'default' => __('Pay via BBSP', 'woocommerce'),
				'desc_tip' => true
			),
			'cardtypes'   => array(
			...
			'logging' => array(
				...
				'default' => 'no',
				'description' => sprintf(
					__('Log all events in <code>woocommerce/logs/blackbaud-woocommerce-bbsp-payment-gateway-log-%s.txt</code><br /><strong>NOTE:</strong>  This will record ALL transaction information.', 'woocommerce'),
					sanitize_file_name( 
						wp_hash('blackbaud')
					)
				)
	...
	*/
	...
	/**
	* Validate the compulsary (required by law) fields for BBSP.
	*
	...
	*/
	function submit_payment($order) {
		...
		// Log request
		$this->log('BBSP Request: ' . print_r($request, true));
		...
		// Log response				
		$this->log('BBSP Response (string): ' . print_r($response_raw, true));
		$this->log('BBSP Response (xml): ' . print_r($response_parsed, true));
